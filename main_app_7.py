...
# 1回目は音声再生処理が実行されるが実行後にtypeエラーがでる
...
@app.route('/')
def index():
    return render_template('main_3.html') # メイン画面を展開する

# 使用説明画面
@app.route('/guide_2.html')
def open_guide():
    return render_template('guide_2.html') # 使用説明画面を展開する
...
def main_action():
    ...
    def text_to_speech(text, language, filename):    
        ...
        # 音声変換したデータをファイルに保存
        text2speech.save(filename + ".mp3") # 2回目以降に音声再生処理を実行するとエラー
    ...
    # 関数実行
    text_to_speech(text, language, filename) # 2回目以降に音声再生処理を実行するとエラー
